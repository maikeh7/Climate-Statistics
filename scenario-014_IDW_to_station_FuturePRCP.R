...
#ONLY USE FOR FUTURE WRF PRECIP PROJECTIONS
#use the other scenario IDW 14 for historical simulations
#has been checked 
require(data.table)
require(dplyr)
library(sp)
...
#VAR: tmax, tmin, or precip. Specify one and spell correctly.
...
Model_Driver = function(days=days, months=months, 
  ...
  #"RCP85" for rcp85, "RCP45" for rcp45
  ...
  for (k in 1:length(months)){
    ...
    for (i in 1:length(days)){
      ...
      climate_data_list = preprocess_climate_data_train(mydata_station, mydata_wrf, VAR="tmax", Lapse_rate)
...
IDW_interpolation = function(climate_data_list, var_of_interest = "Tref",
  ...
  interp_to_stations = gstat::idw(formula = myformula,  locations = ~LON + LAT, data = mydata_wrf.lccDF, 
  ...
  YEAR = rep(YEAR, nrow(LCC_coords))
  MONTH = rep(MONTH, nrow(LCC_coords))
  DAY = rep(DAY, nrow(LCC_coords))
  YMD = data.frame(YEAR=YEAR, MONTH=MONTH, DAY=DAY)
  results_dat = cbind(YMD, mydata_station_latlonDF, LCC_coords)
  return(results_dat)
}
...
import_daily_station_data <- function(VAR = NULL){
  stationdat = read.csv("/raid/mholthui/fromBabbage/data/GHCND/RCP85stationdatTMAX.csv")
  return(stationdat)
}
...
create_results_dir = function(){
  ...
  if(!dir.exists("/raid/mholthui/fromBabbage/IDW_scenario014tmax_RCP85_TOGHCND")){
    dir.create("/raid/mholthui/fromBabbage/IDW_scenario014tmax_RCP85_TOGHCND")
  }
  return("/raid/mholthui/fromBabbage/IDW_scenario014tmax_RCP85_TOGHCND")
}
...
################################################################
preprocess_climate_data_train = function(mydata_station, mydata_wrf, VAR = "tmax", Lapse_rate){
  ...
  YEAR = mydata_wrf.lccDF$YEAR[1]
  MONTH = mydata_wrf.lccDF$MONTH[1]
  DAY = mydata_wrf.lccDF$DAY[1]
  ...
  return(list(mydata_station.lccDF = mydata_station.lccDF, mydata_wrf.lccDF = mydata_wrf.lccDF,
              ...
              mydata_wrfDF_latlon = mydata_wrfDF_latlon, YEAR=YEAR, MONTH=MONTH, DAY=DAY))
}
...
back_transform = function(VAR, Lapse_rate, interp_to_stations){
  ...
  if (VAR == "precip"){
    interp_to_stations$PRECIP_interp_corr = interp_to_stations$var1.pred*((1+Lapse_rate*(interp_to_stations$ELEV - 200)) / (1-Lapse_rate*(interp_to_stations$ELEV - 200)))
